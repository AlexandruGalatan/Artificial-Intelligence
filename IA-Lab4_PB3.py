...
class Problema:
	def __init__(self):
		...
		start = [N, 0, 0] #N din fiecare tip pe malul 0
		final = [0, N, 1] #N din fiecare tip pe malul 1
...
def geth(current_state, target_state):
	...
	score += (current_state[0] - target_state[0])
...
class NodParcurgere:
	...
	def expandeaza(self):
		...
		maxPers = min(self.problema.M, self.nod_graf.info[mal])
		possibleMoves = [x for x in range(1, maxPers + 1)] #trebuie minim 2 pers
	...
	def __str__ (self):
		...
		return f"(Mal0: C{self.nod_graf.info[0]}, M{self.nod_graf.info[0]}; Mal1: C{self.nod_graf.info[1]}, M{self.nod_graf.info[1]}; MalBarca: {self.nod_graf.info[2]})\n"#, parinte={parinte}, f={self.f}, g={self.g})"
