...
def geth(current_state, target_state):
		...
		for i in range(len(current_state)):
			for j in range(len(current_state[i])):
				foundpos = getPos(target_state, current_state[i][j])
				score += (abs(foundpos[0] - i) + abs (foundpos[1] - i))
...
class NodParcurgere:
	...
	def expandeaza(self):
		...
		pos0 = getPos(self.nod_graf.info, "0")

		for miscare in miscari:
			newPos = (pos0[0] + miscare[0], pos0[1] + miscare[1])
			if newPos[0] >= 0 and newPos[0] < len(self.nod_graf.info) and newPos[1] >= 0 and newPos[1] < len(self.nod_graf.info[newPos[0]]):
					succ = copy.deepcopy(self.nod_graf.info)

					aux = succ[newPos[0]][newPos[1]]
					succ[newPos[0]][newPos[1]] = succ[pos0[0]][pos0[1]]
					succ[pos0[0]][pos0[1]] = aux
